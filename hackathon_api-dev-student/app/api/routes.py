...
from app.core.config import MODEL_PATH, API_KEY, API_KEY_HEADER_NAME
...
async def infer_csv_dataset(file: UploadFile = File(...), only_prediction: bool = False):
    """
    CSV 파일 전체에 대한 추론
    ...
    try:
        ...
        # 평가용 - 예측 결과만 반환
        if only_prediction:
            # CSV 문자열을 DataFrame으로 다시 파싱
            df_result = pd.read_csv(StringIO(csv_output))
            # id와 prediction 컬럼만 추출
            csv_output = df_result[['id', 'prediction']].to_csv(index=False)
...
async def validate_model():
    ...
    try:
        accuracy, total, correct, precision, recall, f1_score = inference_service.validate()
...
async def reload_model():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error during reload model: {str(e)}")
